...
do
  case $opt in
    "300x250")
      ...
      python3 app.py --format 300_250 --copy "$copy" --translate "$translate"
    ...
    "300x600")
      ...
      read -p "Would you like it to be translated (y/n)? " translate
      echo "Generating assets..."
      python3 app.py --format 300_600 --copy "$copy" --translate "$translate"
      for o in "${!options[@]}"; do echo "$((o+1)))" "${options[$o]}"; done
      ;;
    "970x250")
      read -p "What do you want it to say (120 chars max)? " copy
      read -p "Would you like it to be translated (y/n)? " translate
      echo "Generating assets..."
      python3 app.py --format 970_250 --copy "$copy" --translate "$translate"
      for o in "${!options[@]}"; do echo "$((o+1)))" "${options[$o]}"; done
      ;;
    "Quit")
      break
